...
import (
	"context"
	"github.com/dstgo/tracker/internal/api"
...
func (a *HttpServer) run() error {
	...
	a.Logger.L().Infof("app boot successfully, http server is listenning at %s, tls enable %t", a.server.Addr, appConf.HttpConf.TlsConf.Enable)
...
func (a *HttpServer) Shutdown() {
	a.once.Do(func() {
		a.Logger.L().Infof("app ready to shutdown")
...
func NewHTTPApp(options ...Options) (*HttpServer, error) {
	...
	if err = LogBanner(app.cfg.BuildMeta, app.cfg.LogConf, app.Logger.L(), "banner.txt"); err != nil {
		return nil, err
	}
